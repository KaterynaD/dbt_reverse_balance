...
- **RecordStatus_col_name** is to highlight the status of the records in the table: **Active** for the records currently active and **Closed** for the reversed records. Population of the column requires additional look up to the target table and can impact the performance. (_Only materialization from a table.)
